...
// Purpose of this code is to create a blend stitched image from I1 and I2.
...
template <typename T>
// function to get affine transformation matrix from corresponding pixels
Mat cal_affine(int ptl_x[], int ptl_y[], int ptr_x[], int ptr_y[], int number_of_points);
// blend stitching two images
...
int main() {
	...
	for (int i = bound_u; i <= bound_b; i++) {
		for (int j = bound_l; j <= bound_r; j++) {
			...
			// neighbor pixels for calculating value by bilinear interpolation
			float y1 = floor(y);
			float y2 = ceil(y);
			float x1 = floor(x);
			float x2 = ceil(x);
