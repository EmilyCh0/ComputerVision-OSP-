...
Mat Laplacianfilter(const Mat input) {
	...
	for (int i = n; i < row + n; i++) {
		for (int j = n; j < col + n; j++) {
			...
			output.at<double>(i-n, j-n) = (double)sum*4.0;
...
Mat Mirroring(const Mat input, int n)
{
	...
	for (int i = n; i < row + n; i++) {
		for (int j = 0; j < n; j++) {
			input2.at<double>(i, j) = input2.at<double>(i, 2 * n - j);	// left border mirroring
		}
		for (int j = col + n; j < col2; j++) {
			input2.at<double>(i, j) = input2.at<double>(i, 2 * col - 2 + 2 * n - j);	// right border mirroring
		}
	}
	for (int j = 0; j < col2; j++) {
		for (int i = 0; i < n; i++) {
			input2.at<double>(i, j) = input2.at<double>(2 * n - i, j);	// top border mirroring
		}
		for (int i = row + n; i < row2; i++) {
			input2.at<double>(i, j) = input2.at<double>(2 * row - 2 + 2 * n - i, j);	// bottom border mirroring
		}
	}
