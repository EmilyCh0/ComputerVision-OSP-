...
template <typename T>
Mat cal_affine(vector<Point2f>& srcPoints, vector<Point2f>& dstPoints, int k, int trials, float thres);
...
int main() {
	...
	Mat A12 = cal_affine<float>(dstPoints, srcPoints,3,50,0.3);
	Mat A21 = cal_affine<float>(srcPoints, dstPoints, 3, 50, 0.3);
...
Mat cal_affine(vector<Point2f>& srcPoints, vector<Point2f>& dstPoints, int k, int trials, float thres) {
	...
	Mat M_trans, temp, affineM, ans;

	int max = srcPoints.size() - 1;
	int maxCnt = 0;
	int cnt = 0;
	...
	for (int i = 0;i < trials ;i++) {
		...
		for (int j = 0;j < k; j++) {
			tmp = rand() % max;
			Point2f pt1 = srcPoints[tmp];
			Point2f pt2 = dstPoints[tmp];
			M.at<T>(2 * j, 0) = pt1.x;		M.at<T>(2 * j, 1) = pt1.y;		M.at<T>(2 * j, 2) = 1;
			M.at<T>(2 *j + 1, 3) = pt1.x;		M.at<T>(2 * j + 1, 4) = pt1.y;		M.at<T>(2 * j + 1, 5) = 1;
			b.at<T>(2 * j) = pt2.x;		b.at<T>(2 * j + 1) = pt2.y;
		}
		...
		for (int j = 0;j < srcPoints.size();j++) {
			Point2f p(affineM.at<float>(0) * srcPoints[j].x + affineM.at<float>(1) * srcPoints[j].y + affineM.at<float>(2), 
				affineM.at<float>(3) * srcPoints[j].x + affineM.at<float>(4) *srcPoints[j].y + affineM.at<float>(5));
			float dist = pow(p.x - dstPoints[j].x,2)+ pow(p.y - dstPoints[j].y, 2);
			if (dist < thres*thres) {
				cnt++;
			}
