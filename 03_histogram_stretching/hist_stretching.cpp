...
int main() {
	...
	Mat stretched = input_gray.clone();	// output
...
void linear_stretching(Mat &input, Mat &stretched, G *trans_func, G x1, G x2, G y1, G y2) {
	...
	for (int i = 0; i < L; i++) {
		if (i >= 0 && i <= x1)
			trans_func[i] = (G)((y1 / (float)x1) * i);	// without (float) result can be distorted
		else if (i > x1 && i <= x2)
			trans_func[i] = (G)(constant * (i - x1) + y1);
		else
			trans_func[i] = (G)((L - 1 - y2) / (float)(L - 1 - x2) * (i - x2) + y2);	//? y2->x2 x2->y2
	}
